...
contract Decode {
    ...
    function decode(string memory _str) external pure returns (string memory) {
        string memory baseString = "data:application/json;base64,";
        string memory substr = substring(
            _str,
            bytes(baseString).length,
            bytes(_str).length
        );
        require((bytes(substr).length % 4) == 0, "Length not multiple of 4");
        bytes memory _bs = bytes(substr);
        uint256 i = 0;
        uint256 j = 0;
